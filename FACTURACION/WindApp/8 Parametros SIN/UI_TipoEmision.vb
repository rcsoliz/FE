...
Public Class UI_TipoEmision : Inherits f_base

#Region "[atributosGlobales]"

    Private usuario As String
    Private clave As String
    Private lista As List(Of tb_tipoemision)
...
#Region "[constructor]"
    ...
    Public Sub New(ByVal usuar As String, ByVal clav As String)
        MyBase.New("TIPOS DE AMBIENTES", "Realice la operación requerida...")
        InitializeComponent()
        usuario = usuar
        clave = clav
        opcion = 1
    ...
    Public Sub New(ByVal usuar As String, ByVal clav As String, ByVal list As List(Of tb_tipoemision), ByVal opc As Integer)
        MyBase.New("TIPOS DE AMBIENTES", "Realice la operación requerida")
        InitializeComponent()
        lista = list
        usuario = usuar
...
#Region "[metodos]"
    ...
    Private Sub getCorrelativo()
        Using oParametroSin As New Negocios.TipoEmisionSinBL(usuario, clave)
    ...
    Private Sub abmSin()
        Using oParametroSin As New Negocios.TipoEmisionSinBL(usuario, clave)
            If validarAlta() Then
                Dim item As New tb_tipoemision With {.codigo = txtCodigo.Text, .descripcion = txtDescripcion.Text}
    ...
    Private Sub inicio()
        If lista.Count > 0 Then
            Using oParametroSin As New Negocios.TipoEmisionSinBL(usuario, clave)
                ...
                Dim item(0) As tb_tipoemision
                item.SetValue(lista.ElementAt(0), 0)

                Dim itemTabla As New tb_tipoemision
