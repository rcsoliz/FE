...
Public Class UI_MotiAnulacion : Inherits f_base

#Region "[atributosGlobales]"

    Private usuario As String
    Private clave As String
    Private lista As List(Of tb_motiAnulacion)
...
#Region "[constructor]"
    ...
    Public Sub New(ByVal usuar As String, ByVal clav As String)
        MyBase.New("MOTIVOS DE ANULACION", "Realice la operación requerida...")
        InitializeComponent()
        usuario = usuar
        clave = clav
        opcion = 1
    ...
    Public Sub New(ByVal usuar As String, ByVal clav As String, ByVal list As List(Of tb_motiAnulacion), ByVal opc As Integer)
        MyBase.New("TIPOS MOTIVOS DE ANULACION", "Realice la operación requerida")
        InitializeComponent()
        lista = list
        usuario = usuar
        clave = clav
...
#Region "[metodos]"
    ...
    Private Sub getCorrelativo()
        Using oParametroSin As New Negocios.MotiAnulacionSinBL(usuario, clave)
    ...
    Private Sub abmSin()
        Using oParametroSin As New Negocios.MotiAnulacionSinBL(usuario, clave)
            If validarAlta() Then
                Dim item As New tb_motiAnulacion With {.codigo = txtCodigo.Text, .descripcion = txtDescripcion.Text}
    ...
    Private Sub inicio()
        If lista.Count > 0 Then
            Using oParametroSin As New Negocios.MotiAnulacionSinBL(usuario, clave)
                ...
                Dim item(0) As tb_motiAnulacion
                item.SetValue(lista.ElementAt(0), 0)

                Dim itemTabla As New tb_motiAnulacion
