...
Public Class UB_UnidMedida : Inherits f_base
...
#Region "[metodos]"
    ...
    Private Sub nuevo()
        ...
        Dim objFormModulo As New UI_UnidMedida(usuario, clave) With {.MdiParent = MdiParent}
        objFormModulo.Show()
        '    Else
    ...
    Private Sub listar()
        ...
        Using oAmbiente As New Negocios.UnidadMedidaBL(usuario, clave)
    ...
    Private Sub modificar()
        ...
        Dim lista As New List(Of Entidades.tb_unidMedida)()
        lista = getLista()
        If lista.Count > 0 Then
            Dim objFormModulo As New UI_UnidMedida(usuario, clave, lista, 2) With {.MdiParent = MdiParent}
            objFormModulo.Show()
        Else
    ...
    Private Sub eliminar()
        ...
        lista = getLista()
        If lista.Count > 0 Then
            Dim objFormModulo As New UI_UnidMedida(usuario, clave, lista, 3) With {.MdiParent = MdiParent}
            objFormModulo.Show()
        Else
    ...
    Private Function getLista() As List(Of Entidades.tb_unidMedida)
        Dim lista As New List(Of Entidades.tb_unidMedida)
        For Each fila As DataGridViewRow In dgv.SelectedRows
            If Not fila.Cells(0).Value Is Nothing Then
                Dim item As Entidades.tb_unidMedida = New Entidades.tb_unidMedida With {.codigo = fila.Cells(0).Value}
                lista.Add(item)
            End If
        Next
        Return lista
