...
Public Class UB_DocuFiscales : Inherits f_base
...
#Region "[metodos]"
    ...
    Private Sub nuevo()
        ...
        Dim objFormModulo As New UI_DocuFiscales(usuario, clave) With {.MdiParent = MdiParent}
        objFormModulo.Show()
        '    Else
    ...
    Private Sub modificar()
        ...
        Dim lista As New List(Of Entidades.tb_docuFiscales)()
        lista = getLista()
        If lista.Count > 0 Then
            Dim objFormModulo As New UI_DocuFiscales(usuario, clave, lista, 2) With {.MdiParent = MdiParent}
            objFormModulo.Show()
        Else
    ...
    Private Sub eliminar()
        ...
        lista = getLista()
        If lista.Count > 0 Then
            Dim objFormModulo As New UI_DocuFiscales(usuario, clave, lista, 3) With {.MdiParent = MdiParent}
            objFormModulo.Show()
        Else
    ...
    Private Function getLista() As List(Of Entidades.tb_docuFiscales)
        Dim lista As New List(Of Entidades.tb_docuFiscales)
        For Each fila As DataGridViewRow In dgv.SelectedRows
            If Not fila.Cells(0).Value Is Nothing Then
                Dim item As Entidades.tb_docuFiscales = New Entidades.tb_docuFiscales With {.codigo = fila.Cells(0).Value}
                lista.Add(item)
            End If
        Next
        Return lista
