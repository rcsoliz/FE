...
Public Class UI_DocuIdentidad : Inherits f_base

#Region "[atributosGlobales]"

    Private usuario As String
    Private clave As String
    Private lista As List(Of tb_docuIdentidad)
...
#Region "[constructor]"
    ...
    Public Sub New(ByVal usuar As String, ByVal clav As String)
        MyBase.New("TIPOS DOCUMENTOS IDENTIDAD", "Realice la operación requerida...")
        InitializeComponent()
        usuario = usuar
        clave = clav
        opcion = 1
    ...
    Public Sub New(ByVal usuar As String, ByVal clav As String, ByVal list As List(Of tb_docuIdentidad), ByVal opc As Integer)
        MyBase.New("TIPOS DOCUMENTOS IDENTIDAD", "Realice la operación requerida")
        InitializeComponent()
        lista = list
        usuario = usuar
...
#Region "[metodos]"
    ...
    Private Sub abmSin()
        Using oParametroSin As New Negocios.DocuIdentidadSinBL(usuario, clave)
            If validarAlta() Then
                Dim item As New tb_docuIdentidad With {.codigo = txtCodigo.Text, .descripcion = txtDescripcion.Text}
    ...
    Private Sub inicio()
        If lista.Count > 0 Then
            Using oParametroSin As New Negocios.DocuIdentidadSinBL(usuario, clave)
                ...
                Dim item(0) As tb_docuIdentidad
                item.SetValue(lista.ElementAt(0), 0)

                Dim itemTabla As New tb_docuIdentidad
