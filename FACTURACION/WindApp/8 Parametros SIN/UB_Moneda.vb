...
Public Class UB_Moneda : Inherits f_base
...
#Region "[constructor]"
    ...
    Public Sub New(ByVal usuar As String, ByVal clav As String, ByVal codMod As Integer, ByVal codProg As Integer)
        MyBase.New("Formulario Datos Monedas", "Realice la operación requerida...")
        InitializeComponent()
        usuario = usuar
        clave = clav
...
#Region "[metodos]"
    ...
    Private Sub nuevo()
        ...
        Dim objFormModulo As New UI_Moneda(usuario, clave) With {.MdiParent = MdiParent}
        objFormModulo.Show()
        '    Else
    ...
    Private Sub listar()
        ...
        Using oAmbiente As New Negocios.MonedaSinBL(usuario, clave)
    ...
    Private Sub modificar()
        ...
        Dim lista As New List(Of Entidades.tb_moneda)()
        lista = getLista()
        If lista.Count > 0 Then
            Dim objFormModulo As New UI_Moneda(usuario, clave, lista, 2) With {.MdiParent = MdiParent}
            objFormModulo.Show()
        Else
    ...
    Private Sub eliminar()
        ...
        lista = getLista()
        If lista.Count > 0 Then
            Dim objFormModulo As New UI_Moneda(usuario, clave, lista, 3) With {.MdiParent = MdiParent}
            objFormModulo.Show()
        Else
    ...
    Private Function getLista() As List(Of Entidades.tb_moneda)
        Dim lista As New List(Of Entidades.tb_moneda)
        For Each fila As DataGridViewRow In dgv.SelectedRows
            If Not fila.Cells(0).Value Is Nothing Then
                Dim item As Entidades.tb_moneda = New Entidades.tb_moneda With {.codigo = fila.Cells(0).Value}
                lista.Add(item)
            End If
        Next
        Return lista
